...
const platforms = [
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24">
        ...
        <rect x="3" y="3" width="18" height="18" rx="5" fill="url(#ig1)" />
  ...
  },
  {
    ...
    value: "pinterest",
    icon: (
      <svg className="w-5 h-5" fill="#BD081C" viewBox="0 0 24 24">
...
export default function SignupPage() {
  ...
  const handlePlatformChange = (platform) => {
    ...
    if (updated.includes(platform)) {
      updated = updated.filter((item) => item !== platform);
    } else {
  ...
  const validateForm = () => {
    ...
    else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(formData.password)) newErrors.password = "Must include uppercase, lowercase & number";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ fullname: "", email: "", password: "", platforms: [] });
      setTimeout(() => navigate("/sign-in"), 2000);
    } catch (error) {
      if (error.payload?.error === "Email already exists") {
        setServerError("Email already taken. Please use another email.");
      } else {
        setServerError(error.payload?.error || error.message || "Signup failed");
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fbf8fc] flex items-center justify-center px-6 py-10 relative overflow-hidden">
      ...
      {/* Card */}
      <div className="w-full max-w-[680px] bg-white border border-gray-200 rounded-3xl shadow-md p-10 sm:p-14">

        {/* Heading */}
        <div className="mb-10">
          <h1 className="text-[42px] font-bold text-[#031635]">Sign Up</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-7">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-base font-semibold text-gray-600 mb-2">Full Name</label>
              <input
                type="text"
                name="fullname"
                ...
                className={`w-full h-14 rounded-xl border px-5 outline-none transition text-base ${
                  errors.fullname ? "border-red-500" : "border-gray-300 focus:border-[#031635]"
                }`}
              />
          ...
          {/* Platforms */}
          <div>
            <h3 className="text-base font-semibold text-gray-600 mb-4">Select Platforms</h3>
            ...
            {errors.platforms && <p className="text-red-500 text-sm mt-2">{errors.platforms}</p>}
          </div>
          ...
          <button
            type="submit"
            className="w-full h-14 rounded-xl bg-[#031635] text-white font-semibold text-lg hover:opacity-95 active:scale-[0.99] transition"
          >
            Create Account
          </button>
        </form>
        ...
        {/* Footer */}
        <div className="mt-10 pt-7 border-t border-gray-100 text-center">
          <p className="text-base text-gray-500">
            Already have an account?{" "}
            <Link to="/sign-in" className="text-[#031635] font-semibold hover:underline">
              Sign In
            </Link>
          </p>
        </div>
